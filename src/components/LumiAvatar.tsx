...
import { LUMI_IMAGES, LUMI_STATUS_TEXT } from '../assets/characterAssets';
import { MOOD_STYLES } from '../utils/lumiMood';
...
export interface LumiAvatarProps {
  ...
  autoSpeak?: boolean;
  /**
   * True when Lumi is actually speaking right now. Screens that drive speech
   * themselves (e.g. the chat uses the parent's lumiVoice) pass this so the
   * avatar can still show the speaking image while she talks.
   */
  speaking?: boolean;
  /**
   * Image shown while Lumi is idle (not speaking / not listening). The chat
   * passes 'greeting'; other screens use 'encouraging' by default. Prevents a
   * lingering 'speaking' base mood from showing the speaking image when Lumi
...
export const LumiAvatar: React.FC<LumiAvatarProps> = ({
  ...
  // lumiVoice.speak() itself — the parent owns audio (avoids double-speak
  ...
  // can be invalidated before it starts (parent's lumiVoice.stop() between
  ...
  // - Lumi is talking  -> speaking image (regardless of the parent's mood)
  ...
  const lumiTalking = isLumiSpeaking || speaking;
  const displayMood: LumiMood = isUserSpeaking
    ? 'listening'
    : lumiTalking
    ? 'speaking'
    : mood === 'speaking'
  ...
  const imageSrc = LUMI_IMAGES[displayMood] || LUMI_IMAGES.greeting;

  const statusLabel = lumiTalking
    ? 'Lumi is speaking...'
    : isUserSpeaking
    ? 'Lumi is listening to you...'
    : mood === 'speaking'
    ? 'Lumi is ready'
    : LUMI_STATUS_TEXT[displayMood] || 'AI IELTS Coach';

  const currentColors = MOOD_STYLES[displayMood] || MOOD_STYLES.greeting;

  return (
    <div
      id="lumi-avatar-container"
    ...
    >
      ...
      <div
        className={`relative w-full overflow-hidden rounded-2xl sm:rounded-3xl border border-[#44475a] bg-[#282a36] shadow-2xl backdrop-blur-xl transition-all duration-500 ${
          isLumiSpeaking ? 'ring-2 ring-[#8be9fd]/60 shadow-[#8be9fd]/20' : isUserSpeaking ? 'ring-2 ring-[#50fa7b]/60 shadow-[#50fa7b]/20' : ''
        }`}
      >
        ...
        <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-3 py-1.5 sm:px-3.5 sm:py-2 bg-gradient-to-b from-[#21222c]/80 via-[#21222c]/40 to-transparent pointer-events-auto">
          <div className="flex items-center gap-2">
            <span className="relative flex h-2 w-2">
              <span
                className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                  isLumiSpeaking ? 'bg-[#8be9fd]' : isUserSpeaking ? 'bg-[#50fa7b]' : 'bg-[#bd93f9]'
                }`}
              />
              <span
            ...
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[#21222c]/80 border border-[#44475a] backdrop-blur-sm">
              <span className="text-xs font-semibold tracking-wider text-[#f8f8f2]">LUMI</span>
          ...
          <div className="flex items-center gap-1.5">
            {onToggleVoice && (
              <button
                ...
                title={voiceEnabled ? 'Mute Lumi Voice' : 'Unmute Lumi Voice'}
              >
        ...
        >
          ...
          <div className="absolute bottom-2 inset-x-2 z-20 flex items-center justify-between gap-2 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-xl bg-[#21222c]/85 border border-[#44475a] backdrop-blur-[2px] text-[#f8f8f2] shadow-md">
            ...
            <SoundWave
              active={isLumiSpeaking || isUserSpeaking}
